...
export default function QuizPage() {
  ...
  const fetchQuizzes = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:8080/api/quizzes?size=${count}`, {
        method: 'GET',
        credentials: 'include',
      });
      ...
      setError('서버 연결에 실패했습니다. 백엔드 서버가 실행 중인지 확인하세요.');
    }
      ...
      <div className="min-h-screen">
        ...
        <div className="min-h-screen flex items-center justify-center px-4 pt-20 pb-8">
          <div className="card max-w-2xl w-full animate-scaleIn">
            ...
            {answerFeedback && (
              <div 
                className={`alert ${answerFeedback === 'correct' ? 'alert-success' : 'alert-error'}`}
                style={{ textAlign: 'center', fontSize: '1.1rem' }}
              >
                {answerFeedback === 'correct' ? (
                  <span>✅ 정답입니다!</span>
                ) : (
                  <span>❌ 틀렸습니다! 정답: <strong>{currentQuiz.answer}</strong></span>
                )}
              </div>
            )}
          ...
          <div className="card max-w-2xl w-full animate-scaleIn">
            ...
            <div className="text-center mb-8">
              ...
              <div 
                ...
                style={{ 
                  background: `linear-gradient(135deg, ${percentage >= 50 ? 'var(--success)' : 'var(--accent)'}, var(--secondary))`,
                    ...
                    <div className="flex-1">
                      ...
                      <div className="flex flex-wrap gap-4 text-sm">
                        <span>
                          ...
                          <span style={{ color: result.isCorrect ? 'var(--success)' : 'var(--error)' }}>
                            {result.userAnswer}
                          </span>
                        </span>
                        {!result.isCorrect && (
                          <span>
                            ...
                            <span style={{ color: 'var(--success)' }}>{result.correctAnswer}</span>
                          </span>
                        )}
                      </div>
                    </div>
